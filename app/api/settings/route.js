...
export async function GET() {
  ...
  const { data } = await supabase.from('user_profiles').select('weekly_email_enabled, weekly_email_day, name, practice_philosophy').eq('email', email).limit(1)
...
export async function POST(request) {
  ...
  const { weekly_email_enabled, weekly_email_day, name, practice_philosophy } = await request.json()
  const supabase = adminSupabase()
  const update = {}
  ...
  if (practice_philosophy !== undefined) update.practice_philosophy = practice_philosophy
