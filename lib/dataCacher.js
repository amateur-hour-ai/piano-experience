...
export async function preloadProfileData(profileEmail, isOwnProfile) {
  try {
    let pieces, schedule, strategies, experiences, activities, userActivities
    let categories = []
    if (isOwnProfile) {
      const [pRes, sRes, strRes, eRes, aRes, catRes, uaRes] = await Promise.all([
        ...
        fetchJson(`/api/user-activities?profile=${enc(profileEmail)}`),
      ])
      pieces = pRes.pieces || []
      ...
      userActivities = uaRes.activities || []
    } else {
      const [pRes, sRes, strRes, eRes, aRes, catRes, uaRes] = await Promise.all([
        ...
        fetchJson(`/api/activity?user_email=${enc(profileEmail)}&limit=20`),
        fetchJson(`/api/categories?profile=${enc(profileEmail)}`),
        fetchJson(`/api/profile/${enc(profileEmail)}/activities`),
      ])
      pieces = pRes.pieces || []
    ...
    await cacheProfileData(profileEmail, {
      pieces, schedule, strategies, experiences, activities, userActivities, categories,
    })
